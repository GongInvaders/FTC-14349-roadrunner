...
public class AU_RED_WH_INTAKE extends LinearOpMode {
...
*/
        ...
        Trajectory warehouse = drive.trajectoryBuilder(lineup.end())
                .lineToLinearHeading(new Pose2d(42.7,-63,0),SampleMecanumDrive.getVelocityConstraint(20, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                        SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL))
                .build();
        Trajectory park = drive.trajectoryBuilder(warehouse.end())
                .strafeLeft(20)
                .build();
        Trajectory grabWarehouseObject = drive.trajectoryBuilder(warehouse.end())
                .forward(11)
                .build();
        Trajectory lineUpWarehouseWall = drive.trajectoryBuilder(grabWarehouseObject.end())
                .strafeRight(2)
                .build();
        Trajectory leaveWarehouse = drive.trajectoryBuilder(grabWarehouseObject.end(),true)
                .lineToLinearHeading(new Pose2d(12.7,-64,0),SampleMecanumDrive.getVelocityConstraint(20, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                        SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL))
                .build();
        Trajectory warehouseHub = drive.trajectoryBuilder(leaveWarehouse.end())
                .lineToLinearHeading(new Pose2d(-10, -39, Math.toRadians(90)))
                .build();
        Trajectory returnToWarehouse = drive.trajectoryBuilder(warehouseHub.end())
                .lineToLinearHeading(new Pose2d(12.7, -64, 0))
                .build();
        ...
        GrabSpin.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        GrabSpin.setPower(1);
        drive.followTrajectory(grabWarehouseObject);
        sleep(2000);
        GrabSpin.setPower(0);
        if(((DistanceSensor) Colour_REV_ColorRangeSensor).getDistance(DistanceUnit.CM) < 3) {
            drive.followTrajectory(lineUpWarehouseWall);
            drive.followTrajectory(leaveWarehouse);
            Spool.setTargetPosition(2500);
            Spool.setPower(1);
            drive.followTrajectory(warehouseHub);
            while (Spool.isBusy()) {
